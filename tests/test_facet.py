...
class TestVoid(unittest.TestCase):
    ...
    def test_center03(self):
        f = Facet([R3(1.0, 1.0, 1.0), R3(3.0, 2.0, 4.0), R3(2.0, 3.0, 2.0)])
        self.assertEqual(f.center().x, 2.0)

    # Площадь грани
    def test_s01(self):
        f = Facet(
            [R3(0.0, 0.0, 0.0), R3(4.0, 0.0, 0.0), R3(4.0, 3.0, 0.0),
             R3(0.0, 3.0, 0.0)])
        self.assertEqual((f.s()), 12.0)

    def test_angle(self):
        f = Facet(
            [R3(1.0, 1.0, 1.0), R3(3.0, 2.0, 4.0), R3(2.0, 3.0, 2.0)])
        self.assertEqual(round(np.degrees(acos(f.angle())), 2), 59.53)

    def test_dist(self):
        f = Facet(
            [R3(1.0, 1.0, 1.0), R3(3.0, 2.0, 4.0), R3(2.0, 3.0, 2.0)])
        self.assertEqual(f.dist(), 0.0)
